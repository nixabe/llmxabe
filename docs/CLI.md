...
| --- | --- | --- | --- |
| `-m, --model <PATH>` | `LLMXABE_MODEL` | the `Qwen3.6-35B-A3B-UD-Q6_K_XL.gguf` path under `~/llama.cpp/models` | GGUF model file to load. The architecture comes from the file's own `general.architecture`: `qwen35moe` (Qwen3.6-35B-A3B) and `qwen35` (Qwen3.8-27B) are implemented, and anything else is refused at preflight by name rather than guessed at. Other quantizations of either are fine. |
